...
class QrCode:
    ...
    def FindVersion(self): # Finds the version of the qr code
        if ((QR_CODE_VERSION_BY_LENGTH.get(int(sqrt(len(self.imData)) / self.cellSize))) == None):
            self.version = QR_CODE_VERSION_BY_LENGTH.get(round(sqrt(len(self.imData)) / self.cellSize)) # Tries to round the calculated length and checks if belongs to version in dictionary, if not, cast it to int instead
        else:
            self.version = QR_CODE_VERSION_BY_LENGTH.get(int(sqrt(len(self.imData)) / self.cellSize))
    def FindCellSize(self): # Finds real cell size in pixels
        self.cellSize = (sqrt(len(self.imData))/QR_CODE_LENGTH_BY_VERSION.get(self.version))          
    
    # Kind of a sequential, step by step approach
    def ScanQrData(self):
        wpercent = (self.length / float(self.im.size[0])) 
        hsize = int((float(self.im.size[1]) * float(wpercent)))
        self.im = self.im.resize((self.length, hsize), Image.Resampling.NEAREST) # Resizes qr image to appropriate resolution given qr version 
